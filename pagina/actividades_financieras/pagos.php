...
//select m.nombre as  medico,p.nombre as  paciente,c.fecha,c.observaciones,c.estado_cita,c.id_cita from pedidos c inner join usuario m on c.id_medico = m.id inner join usuario p on p.id = c.id_cliente
   // $branch=$_SESSION['branch'];
    $query=mysqli_query($con,"select m.nombre as  medico,p.nombre as  paciente,c.id_pedido,c.fecha from pedidos c inner join usuario m on c.id_medico = m.id inner join usuario p on p.id = c.id_cliente  ")or die(mysqli_error());
